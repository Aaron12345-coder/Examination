...
function ServicePackage() {
    ...
    const handleDelete = async (RecordNumber_id) => {
        try {
            await axios.delete(`http://localhost:8000/records/${RecordNumber_id}`);
        ...
        <div className="p-6 max-w-4xl mx-auto">
            ...
            <h2 className="text-2xl font-bold mb-4 text-gray-800">Service Records</h2>
                <table className=" border border-gray-300 rounded-lg shadow-lg w-full">
                    ...
                    <tbody>
                        {records.map((r) => (
                            <tr key={r.RecordNumber_id} className="hover:bg-gray-50">
                                <td className="px-4 py-2 border text-center">{r.RecordNumber_id}</td>
                                <td className="px-4 py-2 border text-center">{r.ServiceDate}</td>
                                <td className="px-4 py-2 border text-center">{r.PackageNumber}</td>
                                <td className="px-4 py-2 border text-center">{r.PlateNumber}</td>
                                <td className="px-4 py-2 border text-center">
                                 <Link to={`/Update/${r.RecordNumber_id}`}>   <button
                                        className="px-4 py-2 text-white bg-blue-500 rounded-md hover:bg-blue-600 transition-all duration-300 mr-2"
                                    >
                                        Update
                                    </button></Link>
                                    <button
                                        onClick={() => handleDelete(r.RecordNumber_id)}
